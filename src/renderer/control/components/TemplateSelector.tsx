  ...
  return (
    <div className="flex flex-wrap gap-1.5 p-2">
      {sorted.map((t) => {
        ...
        const isRec =
          recommended.includes(t.name) || recommended.includes(t.id);

        return (
          <button
            key={t.id}
            onClick={() => setTemplate(t.id)}
            className={`py-1 px-2.5 rounded cursor-pointer text-xs ${isActive ? "border-2 border-accent bg-accent-dark" : "border border-app-600 bg-app-800"} ${isRec ? "text-app-100" : "text-app-200"}`}
          >
            {t.name}
            {isRec && " ★"}
          </button>
